...
const App = {
    ...
    state: {
        ...
        ui: {
            ...
            activeDateMode: 'all', // 현황 페이지의 기간 필터 UI 상태는 유지
            currentView: 'table',
            searchTerm: '',
            searchTimeout: null,
        ...
        // [추가] 중앙 필터 상태 객체
        filters: {
            period: 'all', // 'all', 'year', 'month', 'week', 'day', 'range', 'custom'
            startDate: '',
            endDate: ''
        }
    },
    ...
    init: {
        async start() {
            ...
            // [제거] App.init.setupDateFilterListeners(); // FilterSync가 담당하므로 제거
            await App.data.fetch();
            
            // [추가] FilterSync 모듈 초기화
        ...
        // [제거] setupDateFilterListeners() 함수 전체를 제거합니다.
        // 이 기능은 이제 filter-sync.js에서 담당합니다.
    },
    ...
    filter: {
        apply() {
            ...
            // [수정] 현황 페이지의 기간 필터링 부분을 중앙 상태를 사용하도록 변경
            if (applyDateIndex !== -1 && App.state.filters.period !== 'all') {
        ...
        // [수정] applyDateFilter가 중앙 필터 상태(App.state.filters)를 직접 참조하도록 변경
        applyDateFilter(data, applyDateIndex) {
            const { period, startDate, endDate } = App.state.filters;
            try {
                if (period === 'year' || period === 'month' || period === 'day') {
                    const lengthMap = { year: 4, month: 7, day: 10 };
                    if (startDate) return data.filter(row => String(row[applyDateIndex] || '').slice(0, lengthMap[period]) === startDate.slice(0, lengthMap[period]));
                } else if (period === 'range' || period === 'custom') {
                    if (startDate && endDate) {
                     ...
                     weekEnd.setHours(23,59,59,999);
                      return data.filter(row => {
                            if(!row[applyDateIndex]) return false;
                            const applyDate = new Date(row[applyDateIndex]);
                            return applyDate >= weekStart && applyDate <= weekEnd;
                        });
                }
            } catch(e) {
        ...
        reset(runApplyFilters = true) {
            ...
            // [수정] 중앙 필터 상태를 리셋하고 동기화
            if (window.FilterSync) {
                FilterSync.applyAndSync('reset'); // 'reset'이라는 가상의 source를 전달
            } else {
                 App.state.ui.activeDateMode = 'all';
                 App.state.filters.period = 'all';
        ...
        // 현황 페이지의 동적 UI 생성 로직은 그대로 유지합니다.
        // FilterSync가 이 함수를 호출하여 UI를 그리도록 합니다.
        updateDateFilterUI() {
            ...
            if (App.state.ui.activeDateMode === 'all') {
                ...
                html = `<input type="number" id="dateInput" value="${year}">`;
            } else if (App.state.ui.activeDateMode === 'month') {
                html = `<button class="date-nav-btn" onclick="App.filter.navigateDate(-1)">&lt;</button><input type="month" id="dateInput" value="${year}-${month}"><button class="date-nav-btn" onclick="App.filter.navigateDate(1)">&gt;</button>`;
            } else if (App.state.ui.activeDateMode === 'day') {
                html = `<button class="date-nav-btn" onclick="App.filter.navigateDate(-1)">&lt;</button><input type="date" id="dateInput" value="${year}-${month}-${day}"><button class="date-nav-btn" onclick="App.filter.navigateDate(1)">&gt;</button>`;
            } else if (App.state.ui.activeDateMode === 'range') {
                html = `<input type="date" id="startDateInput"><span style="margin: 0 5px;">-</span><input type="date" id="endDateInput">`;
            }
            container.innerHTML = html;
        },
        
        // 현황 페이지의 날짜 이동 버튼 기능은 그대로 유지합니다.
        // 버튼 클릭 시 FilterSync를 통해 전체 동기화가 이루어집니다.
        navigateDate(direction) {
            ...
            FilterSync.applyAndSync('dashboard');
        }
    },
    ...
    // =========================
    sidebar: {
        // [제거] handlePeriodChange() 함수는 FilterSync가 처리하므로 제거
        
        updateWidgets() {
            // [수정] UI에서 직접 값을 읽는 대신 중앙 필터 상태(App.state.filters)를 사용
            const { period, startDate, endDate } = App.state.filters;
            const applyDateIndex = App.state.data.headers.indexOf('지원일');
            ...
            if (applyDateIndex !== -1 && period !== 'all') {
                const result = App.sidebar.filterByPeriod(filteredApplicants, period, applyDateIndex, startDate, endDate);
            ...
            // [수정] 통계 페이지가 활성화 되어 있을 때만 업데이트 하도록 조건 유지
            if (document.getElementById('stats').classList.contains('active')) {
                App.stats.update();
            }
        },

        // [수정] 중앙 상태 값을 받아 필터링하도록 인수 추가 (startDate, endDate)
        filterByPeriod(data, selectedPeriod, applyDateIndex, startDate, endDate) {
            const now = new Date();
            let filteredData = [...data];
            let label = '전체 기간';

            if (selectedPeriod === 'custom' || selectedPeriod === 'range') {
                if (startDate && endDate) {
                    const start = new Date(startDate);
                    const end = new Date(endDate);
                    end.setHours(23, 59, 59, 999);
        ...
        calculateStats(filteredApplicants) {
            ...
            if (contactResultIndex !== -1) {
                interviewPendingCount = filteredApplicants.filter(row => String(row[contactResultIndex] || '').trim() === '면접확정').length;
            }
            let successRate = 0;
            ...
            if (interviewResultIndex !== -1 && joinDateIndex !== -1) {
                ...
                const joinedApplicants = passedApplicants.filter(row => String(row[joinDateIndex] || '').trim() !== '' && String(row[joinDateIndex] || '').trim() !== '-');
    ...
    stats: {
        ...
        update() {
            ...
            try {
                ...
                if (applyDateIndex !== -1 && period !== 'all') {
                    // [수정] App.sidebar.filterByPeriod를 재사용하여 기간 필터링
                    const result = App.sidebar.filterByPeriod(filteredApplicants, period, applyDateIndex, startDate, endDate);
                    filteredApplicants = result.data;
                    periodLabel = result.label;
                }
        ...
        // [제거] filterByPeriod 함수는 App.sidebar.filterByPeriod로 통합되었으므로 제거
    ...
    charts: {
        ...
        createRouteChart() {
            ...
            if (routeCtx && !App.state.charts.instances.route) {
                App.state.charts.instances.route = new Chart(routeCtx, { type: 'bar', data: { labels: ['데이터 로딩 중...'], datasets: [{ data: [1], backgroundColor: App.config.CHART_COLORS.primary, borderColor: App.config.CHART_COLORS.primary, borderWidth: 1 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true } } } });
            }
        },
        ...
        createPositionChart() {
            ...
            if (positionCtx && !App.state.charts.instances.position) {
                App.state.charts.instances.position = new Chart(positionCtx, { type: 'bar', data: { labels: ['데이터 로딩 중...'], datasets: [{ data: [1], backgroundColor: App.config.CHART_COLORS.success, borderColor: App.config.CHART_COLORS.success, borderWidth: 1 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true } } } });
            }
        },
        ...
        createTrendChart() {
            ...
            if (trendCtx && !App.state.charts.instances.trend) {
                App.state.charts.instances.trend = new Chart(trendCtx, { type: 'line', data: { labels: ['데이터 로딩 중...'], datasets: [{ label: '지원자 수', data: [0], borderColor: App.config.CHART_COLORS.primary, backgroundColor: App.config.CHART_COLORS.primary + '20', tension: 0.4, fill: true }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } } });
            }
        },
        ...
        createRegionChart() {
            ...
            if (regionCtx && !App.state.charts.instances.region) {
                App.state.charts.instances.region = new Chart(regionCtx, { type: 'doughnut', data: { labels: ['데이터 로딩 중...'], datasets: [{ data: [1], backgroundColor: [App.config.CHART_COLORS.primary, App.config.CHART_COLORS.success, App.config.CHART_COLORS.warning, App.config.CHART_COLORS.danger, App.config.CHART_COLORS.orange] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } } });
            }
        },
        ...
        createGenderChart() {
            ...
            if (genderCtx && !App.state.charts.instances.gender) {
                App.state.charts.instances.gender = new Chart(genderCtx, { type: 'pie', data: { labels: ['데이터 로딩 중...'], datasets: [{ data: [1], backgroundColor: [App.config.CHART_COLORS.primary, App.config.CHART_COLORS.warning] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } } });
            }
        },
        ...
        createAgeChart() {
            ...
            if (ageCtx && !App.state.charts.instances.age) {
                App.state.charts.instances.age = new Chart(ageCtx, { type: 'bar', data: { labels: ['데이터 로딩 중...'], datasets: [{ data: [1], backgroundColor: App.config.CHART_COLORS.success, borderColor: App.config.CHART_COLORS.success, borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } } });
            }
        },
        ...
        createRadarChart() {
            ...
            if (radarCtx && !App.state.charts.instances.radar) {
                App.state.charts.instances.radar = new Chart(radarCtx, { type: 'radar', data: { labels: ['지원자 수', '면접확정률', '합격률', '입과율'], datasets: [{ label: '데이터 로딩 중...', data: [1, 1, 1, 1], borderColor: App.config.CHART_COLORS.primary, backgroundColor: App.config.CHART_COLORS.primary + '30', pointBackgroundColor: App.config.CHART_COLORS.primary, pointBorderColor: '#fff', pointHoverBackgroundColor: '#fff', pointHoverBorderColor: App.config.CHART_COLORS.primary }] }, options: { responsive: true, maintainAspectRatio: false, elements: { line: { borderWidth: 3 } }, scales: { r: { angleLines: { display: false }, suggestedMin: 0, suggestedMax: 100 } } } });
            }
        },
        ...
        createScatterChart() {
            ...
            if (scatterCtx && !App.state.charts.instances.scatter) {
                App.state.charts.instances.scatter = new Chart(scatterCtx, { type: 'scatter', data: { datasets: [{ label: '나이-합격률 관계', data: [{ x: 25, y: 50 }], backgroundColor: App.config.CHART_COLORS.orange, borderColor: App.config.CHART_COLORS.orange, pointRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { type: 'linear', position: 'bottom', title: { display: true, text: '나이' } }, y: { title: { display: true, text: '합격률 (%)' }, min: 0, max: 100 } } } });
            }
        },
        ...
        updateAgeChart(filteredData) {
            ...
            const ageGroupData = { '20대 이하': 0, '30대': 0, '40대': 0, '50대': 0, '60대 이상': 0 };
            filteredData.forEach(row => {
        ...
        updateRadarChart(filteredData) {
            ...
            const colors = [App.config.CHART_COLORS.primary, App.config.CHART_COLORS.success, App.config.CHART_COLORS.warning, App.config.CHART_COLORS.danger, App.config.CHART_COLORS.orange];
            const datasets = routes.slice(0, 5).map((route, index) => {
                ...
                const maxApplicants = Math.max(...routes.slice(0, 5).map(r => filteredData.filter(row => String(row[routeIndex] || '').trim() === r).length));
                const normalizedApplicants = maxApplicants > 0 ? (totalApplicants / maxApplicants) * 100 : 0;
                return { label: route, data: [normalizedApplicants, interviewConfirmRate, passRate, joinRate], borderColor: colors[index], backgroundColor: colors[index] + '30', pointBackgroundColor: colors[index], pointBorderColor: '#fff', pointHoverBackgroundColor: '#fff', pointHoverBorderColor: colors[index] };
            });
        ...
        updateScatterChart(filteredData) {
            ...
            const scatterData = Object.entries(ageGroups).filter(([age, data]) => data.total >= 2).map(([age, data]) => ({ x: parseInt(age), y: Math.round((data.passed / data.total) * 100) }));
    ...
    efficiency: {
        ...
        updateAll() {
            // [수정] UI에서 직접 값을 읽는 대신 중앙 필터 상태를 사용하는 getFilteredData() 호출
            const filteredData = App.efficiency.getFilteredData();
        ...
        // [수정] 중앙 필터 상태(App.state.filters)를 사용하도록 변경
        getFilteredData() {
            const { period, startDate, endDate } = App.state.filters;
            const applyDateIndex = App.state.data.headers.indexOf('지원일');
            ...
            if (applyDateIndex !== -1 && period !== 'all') {
                const result = App.sidebar.filterByPeriod(App.state.data.all, period, applyDateIndex, startDate, endDate);
                filteredData = result.data;
            }
        ...
        update(filteredData = null) {
            if (!filteredData) {
                filteredData = App.efficiency.getFilteredData();
            }
        ...
        calculateStats(filteredData, categoryIndex) {
            ...
            filteredData.forEach(row => {
                ...
                if (!stats[category]) {
                    stats[category] = { total: 0, contacted: 0, interviewConfirmed: 0, passed: 0, joined: 0 };
                }
                stats[category].total++;
                if (contactResultIndex !== -1) {
                    const contactResult = String(row[contactResultIndex] || '').trim();
                    if (contactResult !== '' && contactResult !== '-') stats[category].contacted++;
                    if (contactResult === '면접확정') stats[category].interviewConfirmed++;
                }
                if (interviewResultIndex !== -1) {
                    if (String(row[interviewResultIndex] || '').trim() === '합격') stats[category].passed++;
                }
                if (joinDateIndex !== -1) {
                    const joinDate = String(row[joinDateIndex] || '').trim();
                    if (joinDate !== '' && joinDate !== '-') stats[category].joined++;
                }
            });
            return stats;
        },
        ...
        renderTable(stats, categoryName) {
            ...
            let tableHtml = `<div style="overflow-x: auto;"><table class="efficiency-table" style="width: 100%; border-collapse: collapse; font-size: 0.9rem;"><thead><tr style="background: var(--main-bg);"><th style="padding: 12px; text-align: left; border-bottom: 2px solid var(--border-color); font-weight: 600;">${categoryName}</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">총 지원자</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">면접확정</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">합격자</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">입과자</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">면접확정률</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">합격률</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600; color: var(--accent-orange);">최종 입과율</th><th style="padding: 12px; text-align: center; border-bottom: 2px solid var(--border-color); font-weight: 600;">효율성 점수</th></tr></thead><tbody>`;
            ...
            dataArray.forEach((item, index) => {
                ...
                tableHtml += `<tr class="${rankClass}" style="border-bottom: 1px solid var(--border-color); transition: all 0.2s ease;" onmouseover="this.style.transform='translateX(2px)'" onmouseout="this.style.transform='translateX(0)'"><td style="padding: 12px; font-weight: 600; color: ${rankColor};">${rankIcon} ${item.name}</td><td style="padding: 12px; text-align: center; font-weight: 500;">${item.total.toLocaleString()}</td><td style="padding: 12px; text-align: center;">${item.interviewConfirmed}</td><td style="padding: 12px; text-align: center;">${item.passed}</td><td style="padding: 12px; text-align: center;">${item.joined}</td><td style="padding: 12px; text-align: center;">${item.interviewConfirmRate}%</td><td style="padding: 12px; text-align: center;">${item.passRate}%</td><td style="padding: 12px; text-align: center; font-weight: bold; color: var(--accent-orange);">${item.joinRate}%</td><td style="padding: 12px; text-align: center; font-weight: bold; color: ${rankColor}; font-size: 1.1rem;">${item.efficiencyScore}</td></tr>`;
            });
            tableHtml += `</tbody></table></div><div style="margin-top: 15px; padding: 15px; background: var(--main-bg); border-radius: 8px; font-size: 0.85rem; color: var(--text-secondary);"><strong>📊 효율성 점수 계산법:</strong> (입과율 × 0.4) + (합격률 × 0.3) + (면접확정률 × 0.2) + (총지원자수 가중치 × 0.1)</div>`;
    ...
    utils: {
            ...
            if (App.state.ui.currentSortColumn && App.state.ui.currentSortDirection) {
                ...
                if (sortIndex !== -1) {
                    data.sort((a, b) => {
                        ...
                        if (App.state.ui.currentSortColumn === '지원일' || App.state.ui.currentSortColumn.includes('날짜') || App.state.ui.currentSortColumn.includes('날자') || App.state.ui.currentSortColumn.includes('입과일')) {
        ...
        getErrorMessage(status) {
            if (status === 404) { return '데이터를 찾을 수 없습니다. 관리자에게 문의하세요.'; }
            else if (status === 500) { return '서버에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해주세요.'; }
            else if (status === 403) { return '데이터에 접근할 권한이 없습니다.'; }
            else { return `서버 오류 (${status})`; }
        },

        enhanceAccessibility() {
            try {
                const header = document.querySelector('.main-header');
                if (header) { header.setAttribute('role', 'banner'); header.setAttribute('aria-label', '메인 헤더 영역'); }
                const sidebar = document.querySelector('.sidebar');
                if (sidebar) { sidebar.setAttribute('role', 'navigation'); sidebar.setAttribute('aria-label', '주 메뉴 네비게이션'); }
                const mainContent = document.querySelector('.content-area');
                if (mainContent) { mainContent.setAttribute('role', 'main'); mainContent.setAttribute('aria-label', '메인 콘텐츠 영역'); }
                ...
                if (filterBar) { filterBar.setAttribute('role', 'search'); filterBar.setAttribute('aria-label', '지원자 필터링 도구'); }
        ...
        createProgressBar(percentage = 0, text = '로딩 중...') {
            return `<div class="progress-container"><div class="progress-bar"><div class="progress-fill" style="width: ${percentage}%"></div></div><div class="progress-text"><span class="progress-percentage">${percentage}%</span> ${text}</div></div>`;
        },

        createSkeletonTable() {
            const skeletonRows = Array.from({length: 8}, (_, i) => `<tr class="skeleton-row"><td><div class="skeleton-cell short"></div></td><td><div class="skeleton-cell medium"></div></td><td><div class="skeleton-cell long"></div></td><td><div class="skeleton-cell short"></div></td><td><div class="skeleton-cell medium"></div></td><td><div class="skeleton-cell long"></div></td><td><div class="skeleton-cell medium"></div></td></tr>`).join('');
            return `<div class="skeleton-container"><table class="skeleton-table"><thead><tr><th>구분</th><th>이름</th><th>연락처</th><th>지원루트</th><th>모집분야</th><th>지원일</th><th>상태</th></tr></thead><tbody>${skeletonRows}</tbody></table></div>`;
        },
